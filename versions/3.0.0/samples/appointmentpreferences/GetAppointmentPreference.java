			...
			if (response.isExpected())
			{
				...
				if (responseHandler instanceof ResponseWrapper)
				{
					...
					if (appointmentPreferences != null)
					{
						...
						System.out.println("AppointmentPreference whenDurationExceeds : " + appointmentPreferences.getWhenDurationExceeds().getValue());
						...
						Map<String, Object> dealRecordConfiguration = appointmentPreferences.getDealRecordConfiguration();
						if (dealRecordConfiguration != null)
						{
							for (Map.Entry<String, Object> entry : dealRecordConfiguration.entrySet())
							{
								if (entry.getKey().equalsIgnoreCase("layout"))
								{
									com.zoho.crm.api.appointmentpreference.Layout layout = (Layout) entry.getValue();
									System.out.println("Layout Id :" + layout.getId());
									System.out.println("LayoutName : " + layout.getAPIName());
								}
								if (entry.getKey().equalsIgnoreCase("field_mappings"))
								{
									@SuppressWarnings("unchecked")
									List<com.zoho.crm.api.appointmentpreference.FieldMappings> fieldMappings = (List<com.zoho.crm.api.appointmentpreference.FieldMappings>) entry.getValue();
