...
public class Utility
{
	...
	{
		String apiType = field.getDataType().getValue();
		...
		else if (apiType.equalsIgnoreCase(Constants.FORMULA))
		{
			...
			{
				String returnType = ((Formula) field.getFormula()).getReturnType().getValue();
		...
		if (field.getDataType().getValue().equalsIgnoreCase(Constants.PICKLIST) && (field.getPickListValues() != null && field.getPickListValues().size() > 0))
		{
	...
	public String getUserName() throws Exception
	{
		...
		APIResponse<com.zoho.crm.api.users.ResponseHandler> response = usersOperations.getUsers(paramInstance, headerInstance);
		if (response != null)
		{
		...
		return userName + ":" + getUserOrgID();
	}
	
	private String getUserOrgID() throws Exception
	{
		OrgOperations orgOperations = new OrgOperations();
		APIResponse<com.zoho.crm.api.org.ResponseHandler> response = orgOperations.getOrganization();
		if (response != null)
		{
			if (response.isExpected())
			{
